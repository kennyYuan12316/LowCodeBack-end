...
using HSZ.Tenant.Entitys;
using HSZ.Tenant.Entitys.Dtos;
...
namespace HSZ.Tenant
{
    ...
    public class TenantService : IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> GetList([FromQuery] TenantListQuery input)
        {
            ...
            var list = await _tenantRepository.AsQueryable().Where(queryWhere).OrderBy(x => x.CreatorTime, OrderByType.Desc).ToPagedListAsync(input.currentPage, input.pageSize);
        ...
        {
            var output = (await _tenantRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null)).Adapt<TenantInfoOutput>();
            return output;
        }
        ...
        {
            var data = await _tenantRepository.GetFirstAsync(x => x.EnCode == encode && x.DeleteMark == null);
            if (data == null)
                throw HSZException.Oh(ErrorCode.Zh10000);
            if (DateTime.Now > data.ExpiresTime)
                throw HSZException.Oh(ErrorCode.Zh10001);
        ...
        {
            if (await _tenantRepository.IsAnyAsync(x => x.EnCode == input.enCode && x.DeleteMark == null))
                throw HSZException.Oh(ErrorCode.Zh10002);
            var entity = new TenantEntity()
            {
                FullName = input.fullName,
                EnCode = input.enCode,
                CompanyName = input.companyName,
                DbName = "zlcloud_" + Common.Util.PinyinUtil.PinyinString(input.companyName),
        ...
        {
            if (await _tenantRepository.IsAnyAsync(x => x.Id != id && x.EnCode == input.enCode && x.DeleteMark == null))
                throw HSZException.Oh(ErrorCode.Zh10002);
            var entity = new TenantEntity()
            {
                FullName = input.fullName,
            ...
            var isOk = await _tenantRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
            if (isOk < 1)
                throw HSZException.Oh(ErrorCode.COM1000);
        }
        ...
        {
            var entity = await _tenantRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
            if (entity == null)
        ...
        private async Task<int> Create(TenantEntity entity)
        {
            ...
            {
                Db.BeginTran();
                _ = Task.Run(() => { _ = CreateDB(entity.DbName); });
                var isOk = await _tenantRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
                Db.CommitTran();
                return isOk;
            }
            catch (Exception)
        ...
        private async Task CreateDB(string dbName)
        {
            try
            {
                var dbinitName = App.Configuration["HSZ_DB_init:DBName"];
                var dbinitConnString = App.Configuration["HSZ_DB_init:ConnectionString"];
                var dbinitType = App.Configuration["HSZ_DB_init:ConnectionType"];
        ...
        private async Task<int> Delete(TenantEntity entity, int flag)
        {
            ...
            return await _tenantRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        }
