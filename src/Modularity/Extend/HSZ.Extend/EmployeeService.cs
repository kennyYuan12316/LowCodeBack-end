...
using HSZ.Extend.Entitys.Dto.Employee;
...
namespace HSZ.Extend
{
    ...
    public class EmployeeService : IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> GetList([FromQuery] EmployeeListQuery input)
        {
            ...
            var list = await _employeeRepository.AsQueryable().Where(whereLambda)
        ...
        {
            var data = await _employeeRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
            return data;
        }
        ...
        {
            await _employeeRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
        }
        ...
        {
            await _employeeRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        }
        ...
        {
            var entity = await _employeeRepository.GetFirstAsync(x=>x.Id==id&& x.DeleteMark==null);
            await _employeeRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        }
        ...
        public async Task<dynamic> ImportData_Api([FromBody] ImportDataInput input)
        {
            ...
            foreach (var item in input.list)
            {
                try
                {
                    ...
                    var isOk = await _employeeRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
                    if (isOk<1)
                    {
        ...
        public dynamic TemplateDownload()
        {
            ...
            return new { name = "职员信息模板.xlsx", url = "/api/file/Download?encryption=" + DESCEncryption.Encrypt(_userManager.UserId + "|职员信息模板.xlsx","HSZ") };
        }
        #endregion
        ...
        {
            return await _employeeRepository.AsQueryable().Where(x => x.DeleteMark == null).OrderBy(x => x.CreatorTime, OrderByType.Desc).ToListAsync();
        }
        ...
        private async Task<List<EmployeeEntity>> GetPageListData(EmployeeListQuery input)
        {
            ...
            var list = await _employeeRepository.AsQueryable().Where(whereLambda).OrderBy(x => x.CreatorTime, OrderByType.Desc).ToPagedListAsync(input.currentPage, input.pageSize);
