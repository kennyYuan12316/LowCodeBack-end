...
namespace HSZ.Extend
{
    ...
    /// 描 述：订单管理
    /// </summary>
    [ApiDescriptionSettings(Tag = "Extend", Name = "CrmOrder", Order = 200)]
    ...
    public class OrderService : IDynamicApiController, ITransient
    {
        ...
        private readonly ISqlSugarRepository<OrderEntryEntity> _orderEntryRepository;
        private readonly ISqlSugarRepository<OrderReceivableEntity> _orderReceivableRepository;
        ...
        /// <summary>
        /// 初始化一个<see cref="OrderService"/>类型的新实例
        /// </summary>
        public OrderService(ISqlSugarRepository<OrderEntity> orderRepository,
            ISqlSugarRepository<OrderEntryEntity> orderEntryRepository,
            ISqlSugarRepository<OrderReceivableEntity> orderReceivableRepository,
            ...
            IFlowEngineService flowEngineService)
        {
            ...
            _orderReceivableRepository = orderReceivableRepository;
        ...
        /// <returns></returns>
        [HttpGet("")]
        public async Task<dynamic> GetList([FromQuery] OrderListQuery input)
        {
            var queryWhere = LinqExpression.And<OrderListOutput>();
            ...
            if (input.startTime.IsNotEmptyOrNull() && input.endTime.IsNotEmptyOrNull())
            {
                ...
                queryWhere = queryWhere.And(x => SqlFunc.Between(x.orderDate, start, end));
            }
            //数据权限过滤
            if (_userManager.User.IsAdministrator == 0)
            {
                authorizeWhere = await _userManager.GetConditionAsync<OrderListOutput>("F4FCA43A-B563-4149-AB16-486589CFC154");
            }
            var pageList = await _orderRepository.AsSugarClient().Queryable<OrderEntity, UserEntity, FlowTaskEntity>((a, b, c) => new JoinQueryInfos(JoinType.Left, a.CreatorUserId == b.Id, JoinType.Left, a.Id == c.Id)).Where((a, b) => a.DeleteMark == null).Select((a, b, c) => new OrderListOutput()
            {
            ...
            }).MergeTable().Where(queryWhere).Where(authorizeWhere)
            .OrderBy(a=>a.sortCode)
            .OrderBy(a => a.creatorTime, OrderByType.Desc).OrderByIF(!string.IsNullOrEmpty(input.keyword), t => t.lastModifyTime, OrderByType.Desc)
            .ToPagedListAsync(input.currentPage, input.pageSize);
            return PageResult<OrderListOutput>.SqlSugarPageResult(pageList);
        }
        ...
        /// <returns></returns>
        [HttpGet("OrderEntry/{id}/Items")]
        public async Task<dynamic> GetEntryList(string id)
        {
            var data = await _orderEntryRepository.AsQueryable().Where(x => x.OrderId == id).OrderBy(x => x.SortCode).ToListAsync();
            var output = data.Adapt<List<OrderItemsOutput>>();
        ...
        /// <param name="id">主表Id</param>
        /// <returns></returns>
        [HttpGet("{id}/CollectionPlan")]
        public async Task<dynamic> GetReceivableList(string id)
        {
            var data = await _orderReceivableRepository.AsQueryable().Where(x => x.OrderId == id).OrderBy(o => o.SortCode).ToListAsync();
            var output = data.Adapt<List<OrderCollectionPlanOutput>>();
            return new { list = output };
        }
        ...
        {
            var data = (await _orderRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null)).Adapt<OrderInfoOutput>();
            data.collectionPlanList = (await _orderReceivableRepository.AsQueryable().Where(x => x.OrderId == id).ToListAsync()).Adapt<List<CollectionPlanModel>>();
            data.goodsList = (await _orderEntryRepository.AsQueryable().Where(x => x.OrderId == id).ToListAsync()).Adapt<List<GoodsModel>>();
            data.creatorUserId = await _usersService.GetUserName(data.creatorUserId);
            data.lastModifyUserId = await _usersService.GetUserName(data.lastModifyUserId);
            return data;
        }
        #endregion
        ...
        /// <returns></returns>
        [HttpPost("")]
        public async Task Create([FromBody] OrderCrInput input)
        {
            ...
            await _orderEntryRepository.AsInsertable(orderEntryList).ExecuteCommandAsync();
            await _orderReceivableRepository.AsInsertable(orderReceivableList).ExecuteCommandAsync();
            await _orderRepository.AsInsertable(orderEntity).CallEntityMethod(m => m.Create()).ExecuteCommandAsync();
            if (input.status == 0)
            {
                var flowModuleMark = FlowModuleEnum.CRM_Order;
                var flowEngineEntity = await _flowEngineService.GetInfoByEnCode(flowModuleMark);
                await FlowSubmit(orderEntity.Id, flowEngineEntity);
            }
            FileHelper.CreateFile(JsonHelper.ToList<FileModel>(orderEntity.FileJson));
        }
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task Update(string id, [FromBody] OrderUpInput input)
        {
            var orderEntity = input.Adapt<OrderEntity>();
            var orderEntryList = input.goodsList.Adapt<List<OrderEntryEntity>>();
            var orderReceivableList = input.collectionPlanList.Adapt<List<OrderReceivableEntity>>();
            ...
            await _orderEntryRepository.DeleteAsync(x => x.OrderId == id);
            await _orderReceivableRepository.DeleteAsync(x => x.OrderId == id);
            await _orderEntryRepository.AsInsertable(orderEntryList).ExecuteCommandAsync();
            await _orderReceivableRepository.AsInsertable(orderReceivableList).ExecuteCommandAsync();
            await _orderRepository.AsUpdateable(orderEntity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        ...
        {
            var entity = await _orderRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
            if (entity != null)
            {
                await _orderEntryRepository.DeleteAsync(x => x.OrderId == id);
                await _orderReceivableRepository.DeleteAsync(x => x.OrderId == id);
                await _orderRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
                FileHelper.DeleteFile(JsonHelper.ToList<FileModel>(entity.FileJson));
            }
        }
        #endregion
        ...
        /// <returns></returns>
        /// 
        private DataTable GetDataFilter(DataTable data, string condition)
        {
        ...
        private async Task FlowSubmit(string id, FlowEngineEntity flowEngineEntity)
        {
            ...
            {
                Db.BeginTran();
                var orderEntity = await _orderRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
                #region 业务信息
                orderEntity.CurrentState = FlowTaskStatusEnum.Handle;
                orderEntity.LastModifyTime = DateTime.Now;
                orderEntity.LastModifyUserId = _userManager.UserId;
                ...
                {
                    await _flowTaskService.Submit("", flowEngineEntity.Id, id, _userManager.User.RealName + "的订单示例", 1, orderEntity.OrderCode, orderEntity.Adapt<OrderInfo>(), 0);
                }
                else
                {
                    await _flowTaskService.Submit(id, flowEngineEntity.Id, id, _userManager.User.RealName + "的订单示例", 1, orderEntity.OrderCode, orderEntity.Adapt<OrderInfo>(), 0);
                }
                #endregion
                Db.CommitTran();
            }
        ...
        private async Task FlowRevoke(FlowTaskEntity flowTaskEntity, string flowHandleModel)
        {
            ...
            {
                Db.BeginTran();
                var orderEntity = await _orderRepository.GetFirstAsync(x => x.Id == flowTaskEntity.ProcessId && x.DeleteMark == null);
                #region 业务信息
