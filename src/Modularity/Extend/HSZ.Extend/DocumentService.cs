...
using HSZ.Extend.Entitys.Dto.Document;
...
namespace HSZ.Extend
{
    ...
    public class DocumentService : IDynamicApiController, ITransient
    {
        ...
        {
            var data = (await _documentRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == 0)).Adapt<DocumentInfoOutput>();
            if (data.type == 1)
            {
                data.fullName = data.fullName.Replace("." + data.fileExtension, "");
            }
            return data;
        }
        #endregion
        ...
        {
            if (await _documentRepository.IsAnyAsync(x => x.FullName == input.fullName && x.CreatorUserId == _userManager.UserId && x.Type == 0 && x.DeleteMark != 1))
                throw HSZException.Oh(ErrorCode.COM1004);
            var entity = input.Adapt<DocumentEntity>();
            entity.DeleteMark = 0;
            var isOk = await _documentRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
            if (isOk < 1)
        ...
        {
            if (await _documentRepository.IsAnyAsync(x => x.Id != id && x.Type == input.type && x.FullName == input.fullName && x.DeleteMark != 1))
                throw HSZException.Oh(ErrorCode.COM1004);
            var entity = input.Adapt<DocumentEntity>();
            var isOk = await _documentRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
            if (isOk < 1)
        ...
        {
            if (await _documentRepository.IsAnyAsync(x => x.ParentId == id && x.DeleteMark != 1))
                throw HSZException.Oh(ErrorCode.Ex0006);
            var entity = await _documentRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark != 1);
            if (entity == null)
                throw HSZException.Oh(ErrorCode.COM1005);
            var isOk = await _documentRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
            if (isOk < 1)
        ...
        {
            #region 上传图片
            if (await _documentRepository.IsAnyAsync(x => x.FullName == input.file.FileName && x.Type == 1 && x.DeleteMark != 1))
                throw HSZException.Oh(ErrorCode.D8002);
            Thread.Sleep(1000);
            ...
            var isOk = await _documentRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
            if (isOk < 1)
                throw HSZException.Oh(ErrorCode.D8001);
            #endregion
        }
        ...
        {
            var entity = await _documentRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == 0);
            if (entity == null)
        ...
        {
            var list = await _documentRepository.AsQueryable().Where(m => m.ParentId == id && m.CreatorUserId == _userManager.UserId && m.DeleteMark == 0).ToListAsync();
            foreach (var item in list)
            {
                ...
                var isOk = await _documentRepository.AsUpdateable(item).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
                if (isOk < 1)
                    throw HSZException.Oh(ErrorCode.COM1002);
            }
        ...
        {
            var entity = await _documentRepository.GetFirstAsync(x => x.Id == id);
            entity.DeleteMark = 0;
        ...
        {
            var entity = await _documentRepository.GetFirstAsync(x => x.Id == id);
            var entityTo = await _documentRepository.GetFirstAsync(x => x.Id == toId);
            if (id == toId && entity.Type == 0 && entityTo.Type == 0)
                throw HSZException.Oh(ErrorCode.Ex0002);
            if (entityTo.IsNotEmptyOrNull() && id == entityTo.ParentId && entity.Type == 0 && entityTo.Type == 0)
                throw HSZException.Oh(ErrorCode.Ex0005);
