...
namespace HSZ.Message.Service.Message
{
    /// <summary>
    ...
    public class IMContentService : IIMContentService, ITransient
    {
        ...
        public dynamic GetMessageList(string sendUserId, string receiveUserId, PageInputBase input)
        {
            ...
            var list = _IMContentRepository.AsQueryable().Select(it => new IMContentListOutput { id = it.Id, sendUserId = it.SendUserId, sendTime = it.SendTime, receiveUserId = it.ReceiveUserId, receiveTime = it.ReceiveTime, content = it.Content, contentType = it.ContentType, state = it.State }).MergeTable()
        ...
        {
            return _IMContentRepository.AsQueryable().Where(x => x.State == 0 && x.ReceiveUserId == receiveUserId).ToList().Count;
        }
        ...
        {
            var list = _IMContentRepository.AsSugarClient().Queryable<IMContentEntity>().Where(x => x.ReceiveUserId == receiveUserId).Select(x => new IMContentEntity
            {
            ...
            var list1 = _IMContentRepository.AsQueryable().Where(x => x.ReceiveUserId == receiveUserId).OrderBy(x => x.SendTime, OrderByType.Desc).ToList();
        ...
        {
            return _IMContentRepository.AsSugarClient().Updateable<IMContentEntity>().SetColumns(x => x.State == 1).SetColumns(x => x.ReceiveTime == DateTime.Now).Where(x => x.State == 0 && x.SendUserId == sendUserId && x.ReceiveUserId == receiveUserId).ExecuteCommand();
        }
        ...
        public int SendMessage(string sendUserId, string receiveUserId, string message, string messageType)
        {
            ...
            //写入到会话表中
            var isExist = _imReplyRepository.IsAny(it => it.UserId == sendUserId && it.ReceiveUserId == receiveUserId);
            if (isExist)
            {
                var imReplyEntity = _imReplyRepository.GetSingle(it => it.UserId == sendUserId && it.ReceiveUserId == receiveUserId);
                imReplyEntity.ReceiveTime = entity.SendTime;
                _imReplyRepository.AsUpdateable(imReplyEntity).ExecuteCommand();
            }
            else
            {
                ...
                _imReplyRepository.AsInsertable(imReplyEntity).ExecuteCommand();
            }

            return _IMContentRepository.Insert(entity)?0:1;
        }
    }
}
