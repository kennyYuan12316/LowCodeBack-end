...
namespace HSZ.System.Service.System
{
    ...
    public class ModuleColumnService : IModuleColumnService, IDynamicApiController, ITransient
    {
        ...
        public ModuleColumnService(ISqlSugarRepository<ModuleColumnEntity> moduleColumnRepository,
        ...
        {
            if (await _moduleColumnRepository.IsAnyAsync(x => x.ParentId == id && x.DeleteMark == null))
        ...
        public async Task BatchCreate([FromBody] ModuleColumnActionsBatchInput input)
        {
            ...
            var newDic = await _moduleColumnRepository.AsInsertable(entitys).CallEntityMethod(m => m.Creator()).ExecuteReturnEntityAsync();
        ...
        {
            var entity = await _moduleColumnRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
        ...
        {
            return await _moduleColumnRepository.AsQueryable().Where(x => x.DeleteMark == null).OrderBy(o => o.SortCode).ToListAsync();
        }
        ...
        {
            return await _moduleColumnRepository.AsQueryable().Where(x => x.DeleteMark == null && x.ModuleId == moduleId).OrderBy(o => o.SortCode).OrderBy(x=>x.CreatorTime,OrderByType.Desc).OrderBy(t => t.LastModifyTime, OrderByType.Desc).ToListAsync();
        }
        ...
        {
            return await _moduleColumnRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
        }
        ...
        {
            return await _moduleColumnRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        }
        ...
        {
            return await _moduleColumnRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
        }
        ...
        {
            return await _moduleColumnRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        }
        ...
        public async Task<dynamic> GetUserModuleColumnList(bool isAdmin, string userId)
        {
            ...
            if (!isAdmin)
            {
                ...
                if (!string.IsNullOrEmpty(userInfo.RoleId))
                {
                    ...
                    var items = await _authorizeRepository.AsQueryable().In(a => a.ObjectId, roleId).Where(a => a.ItemType == "column").GroupBy(it => new { it.ItemId }).Select(it => new { it.ItemId }).ToListAsync();
                    if (items.Count == 0) return output;
                    var columns = await _moduleColumnRepository.AsQueryable().In(a => a.Id, items.Select(it => it.ItemId).ToArray()).Where(a => a.EnabledMark==1 && a.DeleteMark == null).Select<ModuleColumnEntity>().OrderBy(q => q.SortCode,OrderByType.Asc).ToListAsync();
                    output = columns.Adapt<List<ModuleColumnOutput>>();
                }
            }
            else
            {
                var buttons = await _moduleColumnRepository.AsQueryable().Where(a => a.EnabledMark == 1 && a.DeleteMark == null).Select<ModuleColumnEntity>().OrderBy(q => q.SortCode).ToListAsync();
