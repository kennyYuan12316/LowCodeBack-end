...
using HSZ.System.Entitys.Dto.System.PrintDev;
...
namespace HSZ.System.Service.System
{
    ...
    public class PrintDevService : IPrintDevService, IDynamicApiController, ITransient
    {
        ...
        public PrintDevService(ISqlSugarRepository<PrintDevEntity> printDevRepository,
            ...
            IFileService fileService, IChangeDataBase dataBaseService,
        ...
        {
            var list = await GetOutPageList(input);
            return list;
        }
...
        [HttpGet("Selector")]
        public async Task<dynamic> GetList_Api([FromQuery] string type)
        {
            ...
            var treeList = result.OrderBy(x=>x.sortCode).Union(list).ToList();
        ...
        public async Task<dynamic> GetFields([FromBody] PrintDevFieldsQuery input)
        {
            ...
            foreach (var item in sqlList)
            {
                ...
                var dataTable = _dataBaseService.GetInterFaceData(link, item.sql, parameter.ToArray());
        ...
        public async Task<dynamic> GetData([FromQuery] PrintDevSqlDataQuery input)
        {
            ...
            var dataTable = _dataBaseService.GetInterFaceData(link, sqlList.FirstOrDefault().sql, parameter.ToArray());
            ...
            for (int i = 1; i < sqlList.Count; i++)
            {
                if (sqlList[i].sql.IsNullOrEmpty())
                    throw HSZException.Oh(ErrorCode.COM1005);
                var childDataTable = _dataBaseService.GetInterFaceData(link, sqlList[i].sql, parameter.ToArray());
        ...
        {
            if (await _printDevRepository.IsAnyAsync(x => x.EnCode == input.enCode && x.DeleteMark == null) || await _printDevRepository.IsAnyAsync(x => x.FullName == input.fullName && x.DeleteMark == null))
        ...
        {
            var entity = await _printDevRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
            if (entity == null)
                throw HSZException.Oh(ErrorCode.COM1005);
...
        [HttpPut("{id}")]
        ...
        {
            if (await _printDevRepository.IsAnyAsync(x => x.Id != id && x.EnCode == input.enCode && x.DeleteMark == null) || await _printDevRepository.IsAnyAsync(x => x.Id != id && x.FullName == input.fullName && x.DeleteMark == null))
                throw HSZException.Oh(ErrorCode.COM1004);
            var entity = input.Adapt<PrintDevEntity>();
        ...
        public async Task ActionsCopy(string id)
        {
            ...
            var isOk = await Create(entity);
            if (isOk < 1)
                throw HSZException.Oh(ErrorCode.WF0002);
        }
        ...
        {
            return await _printDevRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
        }
        ...
        {
            return await _printDevRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        }
        ...
        {
            return await _printDevRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
        }
        ...
        {
            return await _printDevRepository.AsQueryable().Where(x => x.DeleteMark == null).ToListAsync();
        }
        ...
        {
            return await _printDevRepository.AsSugarClient().Queryable<PrintDevEntity, UserEntity, UserEntity, DictionaryDataEntity>((a, b, c, d) => new JoinQueryInfos(JoinType.Left, b.Id == a.CreatorUserId, JoinType.Left, c.Id == a.LastModifyUserId, JoinType.Left, a.Category == d.EnCode)).Select((a, b, c, d) => new
            PrintDevListOutput
            {
                category = a.Category,
                id = a.Id,
        ...
        /// <returns></returns>
        [NonAction]
        public async Task<dynamic> GetOutPageList(PrintDevListInput input)
        {
            var list = await _printDevRepository.AsSugarClient().Queryable<PrintDevEntity, UserEntity, UserEntity, DictionaryDataEntity>((a, b, c,d) =>
        ...
        {
            return await _printDevRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        }
        #endregion
        ...
        private async Task ImportData(PrintDevEntity entity)
        {
            ...
            {
                Db.BeginTran();
                var stor = _printDevRepository.AsSugarClient().Storageable(entity).Saveable().ToStorage(); //存在更新不存在插入 根据主键
                ...
                await _printDevRepository.AsSugarClient().Updateable(entity).ExecuteCommandAsync();//执行更新
                Db.CommitTran();
            }
            catch (Exception ex)
            {
