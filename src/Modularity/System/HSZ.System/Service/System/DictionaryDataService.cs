...
using HSZ.System.Entitys.Dto.System.DictionaryData;
...
namespace HSZ.System.Core.Service.DictionaryData
{
    /// <summary>
    ...
    public class DictionaryDataService : IDictionaryDataService, IDynamicApiController, ITransient
    {
        ...
        public DictionaryDataService(ISqlSugarRepository<DictionaryDataEntity> dictionaryDataRepository, IDictionaryTypeService dictionaryTypeService,
        ...
        {
            if (await _dictionaryDataRepository.IsAnyAsync(x => x.EnCode == input.enCode && x.DictionaryTypeId == input.dictionaryTypeId && x.DeleteMark == null) || await _dictionaryDataRepository.IsAnyAsync(x => x.FullName == input.fullName && x.DictionaryTypeId == input.dictionaryTypeId && x.DeleteMark == null))
                throw HSZException.Oh(ErrorCode.D3003);
        ...
        {
            if (await _dictionaryDataRepository.IsAnyAsync(x => x.EnCode == input.enCode && x.DictionaryTypeId == input.dictionaryTypeId && x.Id != id && x.DeleteMark == null) || await _dictionaryDataRepository.IsAnyAsync(x => x.Id != id && x.FullName == input.fullName && x.DictionaryTypeId == input.dictionaryTypeId && x.DeleteMark == null))
                throw HSZException.Oh(ErrorCode.D3003);
            var entity = input.Adapt<DictionaryDataEntity>();
        ...
        {
            var entity = await _dictionaryDataRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
            if (entity == null)
                throw HSZException.Oh(ErrorCode.D3004);
            if (await _dictionaryDataRepository.IsAnyAsync(o => o.ParentId.Equals(id) && o.DeleteMark == null))
        ...
        public async Task ActionsState(string id)
        {
            ...
            var isOk = await Update(entity);
            if (isOk < 0)
                throw HSZException.Oh(ErrorCode.D1506);
        }
        ...
        public async Task<List<DictionaryDataEntity>> GetList(string dictionaryTypeId)
        {
            ...
            return await _dictionaryDataRepository.AsQueryable().Where(d => d.DictionaryTypeId == entity.Id && d.DeleteMark == null)
        ...
        {
            return await _dictionaryDataRepository.AsQueryable().Where(d => d.DeleteMark == null)
                .OrderBy(o => o.SortCode).OrderBy(o => o.CreatorTime, OrderByType.Desc).OrderBy(o => o.LastModifyTime, OrderByType.Desc).ToListAsync();
        }
        ...
        {
            return await _dictionaryDataRepository.GetFirstAsync(x => x.Id == id && x.DeleteMark == null);
        }
        /// <summary>
        /// 删除
        /// </summary>
        ...
        {
            return await _dictionaryDataRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.Delete()).ExecuteCommandAsync();
        }
        /// <summary>
        /// 创建
        /// </summary>
        ...
        {
            return await _dictionaryDataRepository.AsInsertable(entity).CallEntityMethod(m => m.Creator()).ExecuteCommandAsync();
        }
        /// <summary>
        /// 更新
        /// </summary>
        ...
        {
            return await _dictionaryDataRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).CallEntityMethod(m => m.LastModify()).ExecuteCommandAsync();
        }
        #endregion
        ...
        private async Task ImportData(DictionaryDataExportInput inputList)
        {
            try
            {
                ...
                #region 插入数据
                Db.BeginTran();
                var typeStor = _dictionaryDataRepository.AsSugarClient().Storageable(typeList).Saveable().ToStorage(); //存在更新不存在插入 根据主键
                typeStor.AsInsertable.ExecuteCommand(); //执行插入
                typeStor.AsUpdateable.ExecuteCommand(); //执行更新　
                var dataStor = _dictionaryDataRepository.AsSugarClient().Storageable(dataList).Saveable().ToStorage();//存在更新不存在插入 根据主键
