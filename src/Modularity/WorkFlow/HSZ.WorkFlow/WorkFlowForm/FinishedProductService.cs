...
using HSZ.WorkFlow.Entitys.Dto.WorkFlowForm.FinishedProduct;
...
namespace HSZ.WorkFlow.WorkFlowForm
{
    ...
    public class FinishedProductService : IFinishedProductService, IDynamicApiController, ITransient
    {
        ...
        public FinishedProductService(ISqlSugarRepository<FinishedProductEntity> sqlSugarRepository, 
            ISqlSugarRepository<FinishedProductEntryEntity> sqlItemSugarRepository,
        ...
        {
            var data = (await _sqlSugarRepository.GetFirstAsync(x => x.Id == id)).Adapt<FinishedProductInfoOutput>();
            data.entryList = (await _sqlItemSugarRepository.AsQueryable().Where(x => x.WarehouseId == id).ToListAsync()).Adapt<List<EntryListItem>>();
            return data;
        }
        #endregion
        ...
        /// <param name="itemList"></param>
        /// <returns></returns>
        private async Task Submit(string id, FinishedProductEntity entity, List<FinishedProductEntryEntity> itemList, Dictionary<string, List<string>> candidateList)
        {
            try
            {
                Db.BeginTran();
                ...
                #region 流程信息
                await _flowTaskService.Submit(id, entity.FlowId, entity.Id, entity.FlowTitle, entity.FlowUrgent, entity.BillNo, entity.Adapt<FinishedProductUpInput>(), 0, 0, true,false, candidateList);
                #endregion
