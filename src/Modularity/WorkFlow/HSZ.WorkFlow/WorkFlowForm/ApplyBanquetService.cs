...
using HSZ.WorkFlow.Entitys.Dto.WorkFlowForm.ApplyBanquet;
...
namespace HSZ.WorkFlow.WorkFlowForm
{
    ...
    public class ApplyBanquetService : IApplyBanquetService, IDynamicApiController, ITransient
    {
        ...
        public ApplyBanquetService(ISqlSugarRepository<ApplyBanquetEntity> sqlSugarRepository, IBillRullService billRuleService, 
        ...
        {
            var data = (await _sqlSugarRepository.GetFirstAsync(x => x.Id == id)).Adapt<ApplyBanquetInfoOutput>();
            return data;
        }
        #endregion
        ...
        private async Task Submit(string id, ApplyBanquetEntity entity, Dictionary<string, List<string>> candidateList)
        {
            try
            {
                Db.BeginTran();
                ...
                #region 流程信息
                await _flowTaskService.Submit(id, entity.FlowId, entity.Id, entity.FlowTitle, entity.FlowUrgent, entity.BillNo, entity.Adapt<ApplyBanquetUpInput>(), 0, 0, true,false,candidateList);
                #endregion
