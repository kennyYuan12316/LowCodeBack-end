...
namespace ZJN.Calb
{
    ...
    public class FrameMemberService : ILesService
    {
        ...
        /// <summary>
        /// 初始化一个<see cref="FrameMemberService"/>类型的新实例
        /// </summary>
        public FrameMemberService(ISqlSugarRepository repository, LESServerClient lESServerClient, ISqlSugarRepository<ZjnWmsTrayEntity> zjnBaseTrayRepository, IUserManager userManager)
        {
        ...
        private async void TaskWarehouse_Out(string json)
        {
            ...
            if (request.operationType == "emptyContainer")//emptyContainer【空托】
            {
                string _trayType = request.materialCode;
                string[] _trayTypeA;
                Log.Information($"结构件空托出库2");
                if (_trayType.IndexOf("-") > 0)
                {
                    Log.Information($"结构件空托出库3");
                    _trayTypeA = _trayType.Split('-');
                }
                else
                {
                    Log.Information($"结构件空托出库4");
                    throw new Exception("托盘类型必须带-");
                }
                Log.Information($"结构件空托出库5");
                string _cod = _db.Ado.GetString($"select top 1 F_TrayNo from zjn_wms_tray where F_Type = {_trayTypeA[1]} and F_TrayStates = 0");
                if (_cod.IsNullOrEmpty())
                {
                    Log.Information($"结构件空托出库6");
                    throw new Exception("该类型容器没有可用空托");
                }
                Log.Information($"结构件空托出库7");
                //1.锁定托盘调出入库确认接口
                _db.Ado.ExecuteCommand($"update zjn_wms_tray set F_TrayStates = 2 where F_TrayNo = '{_cod}'");
                InoutConFirmRequest dto = new InoutConFirmRequest();
                //Log.Information($"结构件空托出库8");
                //dto.requestId = YitIdHelper.NextId().ToString();
                //dto.operationType = "emptyContainer";
                //dto.toChannelId = "1642053037569";
                //dto.clientCode = "ZJN04";
                //dto.requestTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                ...
                //dto.lines.Add(new InoutConFirmLine()
                //{
                //    fromLocator = "5001-W103-LK",//request.fromWhouseNo,//"5001-W101-1000",//request.fromLocNo,
                //    toLocator = request.toLocNo,//"5001-W101-003-001",//立库出库口编码
                ...
                dto.lines.Add(new InoutConFirmLine()
                {
                    fromLocator = "5001-W103-LK",//request.fromLocNo,
            ...
            else//production【产品】
            {
                ...
                foreach (System.Data.DataRow rw in dt.Rows)
                {
                    ...
                    //if (request.toLocNo.ToString() == "5001-W103-1000-02" || request.toLocNo.ToString() == "5001-W103-CBJ" || request.toLocNo.ToString() == "5001-W103-CBJ-02")//二楼出库口编号
                    //{
                    //    //3.AGV在库口开始搬运状态返馈接口
        ...
        private async void TaskWarehouse_Into(string json)
        {
            ...
            else//production【产品】
            {
                ...
                {
                    fromLocator = request.fromLocNo,
                    toLocator = "5001-W103-LK",//"ZHWH-W1",//立库编码
                    container = request.diskInfoList[0].trayBarcode,
                    materialBarcode = request.materialCode//"10000048-0"
                });
                Log.Information($"出入库确认，请求入参【{dto.Serialize()}】");
        ...
        private async void TaskWarehouse_Move(string json)
        {
            ...
            for (int i = 0; i < request.diskInfoList.Count; i++)
            {
                if (request.diskInfoList[i].trayBarcode.IsNotEmptyOrNull())
                {
                    ...
                    _request.taskStatus = "start";//开始“start”、暂停“suspend”、完成“complete”
                    _request.agvNum = "AGV001";
                    _request.fromLocatorCode = request.fromLocNo;//"W1-2F-W1001";//"JS-C129-JG1-M34-LY5";
                    _request.locatorCode = request.toLocNo; //"W1-2F-W1002";//"5001-P101-100-001";
                    _request.location = request.fromLocNo; //"W1-2F-W1002-001";//"JS-P404-C01-P404-001";
                    _request.containerCode = _cod;//request.diskInfoList[0].trayBarcode;//"10000012202210WHZG000000000040";
                    var results = _LESServer.StatusFeedBack(_request);
                    results.Wait();
                    var cc = results.Result;
                    ...
                    _request.fromLocatorCode = request.fromLocNo;//"W1-2F-W1001"; //"JS-C129-JG1-M34-LY5";
                    _request.locatorCode = request.toLocNo;//"W1-2F-W1002"; //"5001-P101-100-001";
                    _request.location = request.toLocNo;//"W1-2F-W1002-001"; //"JS-P404-C01-P404-001";
                    _request.containerCode = _cod;//request.diskInfoList[0].trayBarcode; //"10000012202210WHZG000000000040";
                    results = _LESServer.StatusFeedBack(_request);
                    results.Wait();
                    cc = results.Result;
        ...
        private async void TaskWarehouse_Disptch(string json)
        {
            ...
            if (request.operationType == "emptyContainer")//emptyContainer【空托】
            {
                ...
                string _cod = _db.Ado.GetString($"select top 1 F_TrayNo from zjn_wms_tray where F_TrayNo = '{request.materialCode}' and F_TrayStates = 0");
                if (_cod.IsNullOrEmpty())
                {
                    throw new Exception("该类型容器没有可用空托");
                }
            ...
            else//production【产品】
            {
                ...
                _request.locatorCode = "W1-2F-W1002";//"5001-P101-100-001";
                _request.location = "W1-2F-W1002-001";//"JS-P404-C01-P404-001";
                _request.containerCode = request.diskInfoList[0].trayBarcode;//"10000012202210WHZG000000000040";
                var results = _LESServer.StatusFeedBack(_request);
                results.Wait();
                var cc = results.Result;
                Log.Information("AGV搬运2");
