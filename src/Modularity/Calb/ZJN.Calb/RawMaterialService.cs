...
namespace ZJN.Calb
{
    ...
    public class RawMaterialService : ILesService
    {
        ...
        /// <summary>
        /// 初始化一个<see cref="RawMaterialService"/>类型的新实例
        /// </summary>
        public RawMaterialService(ISqlSugarRepository repository, LESServerClient lESServerClient, ISqlSugarRepository<ZjnWmsTrayEntity> zjnBaseTrayRepository, IUserManager userManager)
        {
        ...
        public string CONTAINER_INTO_STOP(string json)
        {
            ...
            switch (_cod)
            {
                case "4":
                case "1":
                    _db.Ado.ExecuteCommand($"update zjn_wms_task set F_TaskState = 4 where F_TaskNo = '{request.orderNo}'");
                    break;
                case "2":
            ...
            else
            {
                switch (request.operationDirection.ToUpper())
                {
                    case "OUT"://出库
                        ...
                        if (request.operationType == "emptyContainer")//【空托】
                        {
                            string _trayType = request.materialCode;
                            string[] _trayTypeA;
                            if (_trayType.IsNullOrEmpty())
                            {
                                throw new Exception("materialCode物料类型为空");
                            }
        ...
        private async void TaskWarehouse_Out(string json)
        {
            ...
            if (request.operationType == "emptyContainer")//emptyContainer【空托】
            {
                ...
                dto.lines.Add(new InoutConFirmLine()
                {
                    fromLocator = "5001-W101-LK",//request.fromLocNo,
            ...
            else//production【产品】
            {
                ...
                foreach (System.Data.DataRow rw in dt.Rows)
                {
                    ...
                    {
                        _toLocator = "5001-W101-003-002";
                    }
                    dto.lines.Add(new InoutConFirmLine()
                    {
                        fromLocator = request.fromWhouseNo,//"5001-W101-1000",//request.fromLocNo,
                        toLocator = _toLocator,//"5001-W101-003-001",//立库出库口编码
                        container = _cod,//request.diskInfoList[0].trayBarcode,
                        materialBarcode = request.materialCode//"10000048-0"
                    });
                    Log.Information($"出入库确认，请求入参【{dto.Serialize()}】");
                    ...
                    if (request.taskType.ToUpper() == "AGV")
                    {
                        //3.AGV在库口开始搬运状态返馈接口
        ...
        private async void TaskWarehouse_Into(string json)
        {
            ...
            Log.Information("入库2");
            if (request.operationType.ToUpper() == "EMPTYCONTAINER")//emptyContainer【空托】
            {
                //任务创建
                //_db.Ado.ExecuteCommand($"insert into zjn_wms_task(F_TaskNo,F_TaskState) values('{request.orderNo}',1)");
                ...
                string _cod = string.Empty;
                _cod = _db.Ado.GetString($"select top 1 F_TaskState from zjn_wms_task where F_TaskNo = '{request.orderNo}'");
                if (string.IsNullOrEmpty(_cod))
                {
                    throw new Exception("任务不存在！");
                }
            ...
            else if (request.operationType.ToUpper() == "PACKING")
            {
                Log.Information("入库3");
                ...
                if (request.taskType.ToUpper() == "AGV")
                {
                    ...
                    _request.locatorCode = request.toLocNo; //"W1-2F-W1002"; //"5001-P101-100-001";
                    _request.location = request.toLocNo; //"W1-2F-W1002-001"; //"JS-P404-C01-P404-001";
                    _request.containerCode = request.diskInfoList[0].trayBarcode; //_cod;//request.diskInfoList[0].trayBarcode; //"10000012202210WHZG000000000040";
                    results = _LESServer.StatusFeedBack(_request);
                    results.Wait();
                    cc = results.Result;
                    Log.Information("AGV搬运3");
                ...
                InoutConFirmRequest dto = new InoutConFirmRequest();
                dto.requestId = YitIdHelper.NextId().ToString();
                dto.operationType = request.operationType;//"production";
                dto.toChannelId = "1642053037569";
                dto.clientCode = "ZJN01";
                dto.requestTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                dto.type = "arrive";
                ...
                {
                    fromLocator = request.fromLocNo,
                    toLocator = "5001-W101-LK",//"ZHWH-W1",//立库编码
                    container = request.diskInfoList[0].trayBarcode,
                    materialBarcode = request.materialCode//"10000048-0"
                });
                Log.Information($"出入库确认，请求入参【{dto.Serialize()}】");
