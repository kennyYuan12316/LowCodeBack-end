...
using HSZ.wms.Interfaces.ZjnWmsSupplier;
...
namespace HSZ.wms.ZjnWmsSupplier
{
    /// <summary>
    /// 供应商管理服务
    /// </summary>
    [ApiDescriptionSettings(Tag = "wms", Name = "WmsSupplier", Order = 200)]
    [Route("api/wms/[controller]")]
    public class ZjnWmsSupplierService : IZjnWmsSupplierService, IDynamicApiController, ITransient
    {
        private readonly ISqlSugarRepository<ZjnWmsSupplierEntity> _zjnWmsSupplierRepository;
        private readonly IUserManager _userManager;
        private readonly SqlSugarScope _db;
        private readonly ISqlSugarRepository<ZjnWmsOperationLogEntity> _zjnPlaneOperationLogRepository;

        /// <summary>
        /// 初始化一个<see cref="ZjnWmsSupplierService"/>类型的新实例
        /// </summary>
        public ZjnWmsSupplierService(ISqlSugarRepository<ZjnWmsSupplierEntity> zjnBaseSupplierRepository, ISqlSugarRepository<ZjnWmsOperationLogEntity> zjnPlaneOperationLogRepository,
            IUserManager userManager)
        {
            _zjnPlaneOperationLogRepository = zjnPlaneOperationLogRepository;
            _zjnWmsSupplierRepository = zjnBaseSupplierRepository;
        ...
        {
            var output = (await _zjnWmsSupplierRepository.GetFirstAsync(p => p.Id == id)).Adapt<ZjnWmsSupplierInfoOutput>();
            return output;
        }
        ...
        {
            var output = await _zjnWmsSupplierRepository.IsAnyAsync(p => p.SupplierNo == SupplierNo && p.IsDelete==0);
            return output;
        }
        ...
        {
            var dataList = new List<ZjnWmsSupplierCrInput>() { new ZjnWmsSupplierCrInput() { } };//初始化 一条空数据
            ...
            ExcelExportHelper<ZjnWmsSupplierCrInput>.Export(dataList, excelconfig, addPath);
        ...
        /// <returns></returns>
        [HttpPost("ImportData")]
        public async Task<dynamic> ImportData(List<ZjnWmsSupplierCrInput> list)
        {
            var res = await ImportSupplierDatas(list);
            var addlist = res.First() as List<ZjnWmsSupplierCrInput>;
            var errorlist = res.Last() as List<ZjnWmsSupplierCrInput>;
            return new ZjnWmsSupplierImportResultOutput() { snum = addlist.Count, fnum = errorlist.Count, failResult = errorlist, resultType = errorlist.Count < 1 ? 0 : 1 };
        }
        ...
        private async Task<object[]> ImportSupplierDatas(List<ZjnWmsSupplierCrInput> list)
        {
            List<ZjnWmsSupplierCrInput> supplierInputList = list;
            ...
            var _zjnBaseSupplierRepositoryList = await _zjnWmsSupplierRepository.GetListAsync();//供应商数据集
            ...
            var zjnBaseSuppList = new List<ZjnWmsSupplierEntity>();
            var userInfo = await _userManager.GetUserInfo();

            foreach (var item in supplierInputList)
            {
                var uentity = new ZjnWmsSupplierEntity();
            ...
            if (zjnBaseSuppList.Any())
            {
                try
                {
                    ...
                    //新增供应商记录
                    var newEntity = await _zjnWmsSupplierRepository.AsInsertable(zjnBaseSuppList).ExecuteReturnEntityAsync();
        ...
        public async Task<dynamic> ExportExceptionData(List<ZjnWmsSupplierCrInput> list)
        {
            var res = await ImportSupplierDatas(list);
            var errorlist = res.Last() as List<ZjnWmsSupplierCrInput>;//错误数据
            ...
            var addPath = FileVariable.TemporaryFilePath + excelconfig.FileName;
            ExcelExportHelper<ZjnWmsSupplierCrInput>.Export(errorlist, excelconfig, addPath);
        ...
        /// <returns></returns>
        [HttpGet("ExportExcel")]
        public async Task<dynamic> ExportExcel([FromQuery] ZjnWmsSupplierListQueryInput input)
        {
            //供应商信息列表
            var userList = new List<ZjnWmsSupplierInfoOutput>();
            var sidx = input.sidx == null ? "F_Id" : input.sidx;

            if (input.dataType == "0")
            {
                userList = await _zjnWmsSupplierRepository.AsSugarClient().Queryable<ZjnWmsSupplierEntity>()
                ...
                .Select((a) => new ZjnWmsSupplierInfoOutput
                {
                    id = a.Id,
            ...
            var addPath = FileVariable.TemporaryFilePath + excelconfig.FileName;
            ExcelExportHelper<ZjnWmsSupplierInfoOutput>.Export(userList, excelconfig, addPath);
		...
		/// <returns></returns>
        [HttpGet("")]
        public async Task<dynamic> GetList([FromQuery] ZjnWmsSupplierListQueryInput input)
        {
            var sidx = input.sidx == null ? "F_Id" : input.sidx;
            var data = await _zjnWmsSupplierRepository.AsSugarClient().Queryable<ZjnWmsSupplierEntity>()
                .Where(x=> x.IsDelete==0)
                .WhereIF(!string.IsNullOrEmpty(input.F_SupplierNo), a => a.SupplierNo.Contains(input.F_SupplierNo))
                .WhereIF(!string.IsNullOrEmpty(input.F_SupplierName), a => a.SupplierName.Contains(input.F_SupplierName))
                .Select((a) => new ZjnWmsSupplierListOutput{
                F_Id = a.Id,
    ...
    F_ModifiedTime = a.ModifiedTime,
            ...
            return PageResult<ZjnWmsSupplierListOutput>.SqlSugarPageResult(data);
        }
        ...
        /// <returns></returns>
        [HttpPost("")]
        public async Task Create([FromBody] ZjnWmsSupplierCrInput input)
        {
            if(await this.ExistSupplierNo(input.supplierNo)) throw HSZException.Oh(ErrorCode.COM1004);

            var userInfo = await _userManager.GetUserInfo();
            var entity = input.Adapt<ZjnWmsSupplierEntity>();
            entity.Id = YitIdHelper.NextId().ToString();
            entity.CreateUser = _userManager.UserId;
            entity.CreateTime = DateTime.Now;
            entity.IsDelete = 0;
            var isOk = await _zjnWmsSupplierRepository.AsInsertable(entity).IgnoreColumns(ignoreNullColumn: true).ExecuteCommandAsync();
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task Update(string id, [FromBody] ZjnWmsSupplierUpInput input)
        {
            var entityl = await _zjnWmsSupplierRepository.GetFirstAsync(p => p.Id.Equals(input.id));
            var entity = input.Adapt<ZjnWmsSupplierEntity>();
            ...
            try
            {
                ...
                _db.BeginTran();
                //修改数据
                var isOk = await _zjnWmsSupplierRepository.AsUpdateable(entity).ExecuteCommandAsync();
                //新增日子记录
        ...
        {
            ZjnWmsOperationLogEntity operationLogEntity = new ZjnWmsOperationLogEntity();
            var entity = await _zjnWmsSupplierRepository.GetFirstAsync(p => p.Id.Equals(id));
