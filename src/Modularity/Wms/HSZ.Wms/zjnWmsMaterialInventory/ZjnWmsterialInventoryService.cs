...
using HSZ.wms.Interfaces.zjnWmsMaterialInventory;
using HSZ.wms.Entitys.Dto.zjnWmsMaterialInventory;


namespace HSZ.wms.zjnWmsMaterialInventory
{
    /// <summary>
    /// 立库库存信息服务
    /// </summary>
    [ApiDescriptionSettings(Tag = "wms", Name = "zjnWmsMaterialInventory", Order = 200)]
    [Route("api/wms/[controller]")]
    public class ZjnWmsterialInventoryService : IZjnWmsMaterialInventoryService, IDynamicApiController, ITransient
    {
        ...
        /// <summary>
        /// 初始化一个<see cref="ZjnWmsterialInventoryService"/>类型的新实例
        /// </summary>
        public ZjnWmsterialInventoryService(ISqlSugarRepository<ZjnWmsMaterialInventoryEntity> zjnBaseMaterialInventoryRepository,
            IUserManager userManager)
        {
            _zjnWmsMaterialInventoryRepository = zjnBaseMaterialInventoryRepository;
        ...
        {
            var output = (await _zjnWmsMaterialInventoryRepository.GetFirstAsync(p => p.Id == id)).Adapt<zjnWmsMaterialInventoryInfoOutput>();
            return output;
        }
        ...
        public async Task<dynamic> GetList([FromQuery] zjnWmsMaterialInventoryListQueryInput input)
        {
            ...
            var data = await _zjnWmsMaterialInventoryRepository.AsSugarClient().Queryable<ZjnWmsMaterialInventoryEntity>()
                ...
                .WhereIF(!string.IsNullOrEmpty(input.F_ProductsContainer), a => a.ProductsContainer.Contains(input.F_ProductsContainer))
                .Select((a
) => new zjnWmsMaterialInventoryListOutput
{
    ...
    F_ProductsContainer = a.ProductsContainer
        ...
        public async Task Create([FromBody] zjnWmsMaterialInventoryCrInput input)
        {
            ...
            var entity = input.Adapt<ZjnWmsMaterialInventoryEntity>();
            entity.Id = YitIdHelper.NextId().ToString();

            var isOk = await _zjnWmsMaterialInventoryRepository.AsInsertable(entity).IgnoreColumns(ignoreNullColumn: true).ExecuteCommandAsync();
...
) => new zjnWmsMaterialInventoryListOutput
{
    ...
    F_ProductsLocation = a.ProductsLocation,
        ...
        {
            var entitys = await _zjnWmsMaterialInventoryRepository.AsQueryable().In(it => it.Id, ids).ToListAsync();
            if (entitys.Count > 0)
            {
                try
                {
                    ...
                    _db.BeginTran();
                    //批量删除立库库存信息
                    await _zjnWmsMaterialInventoryRepository.AsDeleteable().In(d => d.Id, ids).ExecuteCommandAsync();
                    //关闭事务
                    _db.CommitTran();
                }
                catch (Exception)
        ...
        {
            var entity = input.Adapt<ZjnWmsMaterialInventoryEntity>();
            var isOk = await _zjnWmsMaterialInventoryRepository.AsUpdateable(entity).IgnoreColumns(ignoreAllNullColumns: true).ExecuteCommandAsync();
        ...
        {
            var entity = await _zjnWmsMaterialInventoryRepository.GetFirstAsync(p => p.Id.Equals(id));
            _ = entity ?? throw HSZException.Oh(ErrorCode.COM1005);
            var isOk = await _zjnWmsMaterialInventoryRepository.AsDeleteable().Where(d => d.Id == id).ExecuteCommandAsync();
