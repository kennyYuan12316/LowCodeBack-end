...
namespace HSZ.wms.ZjnRoadwayGroup
{
    /// <summary>
    /// 巷道分组信息服务
    /// </summary>
    [ApiDescriptionSettings(Tag = "wms",Name = "ZjnRoadwayGroup", Order = 200)]
    [Route("api/wms/[controller]")]
    public class ZjnRoadwayGroupService : IZjnRoadwayGroupService, IDynamicApiController, ITransient
    {
        private readonly ISqlSugarRepository<ZjnRoadwayGroupEntity> _zjnRoadwayGroupRepository;
        private readonly ISqlSugarRepository<ZjnRoadwayGroupDetailsEntity> _zjnRoadwayGroupDetailsRepository;
        private readonly ISqlSugarRepository<ZjnWmsRoadwayInboundPlanEntity> _zjnRoadwayInboundPlanRepository;
        private readonly IUserManager _userManager;
        private readonly SqlSugarScope _db;

        /// <summary>
        /// 初始化一个<see cref="ZjnRoadwayGroupService"/>类型的新实例
        /// </summary>
        public ZjnRoadwayGroupService(ISqlSugarRepository<ZjnRoadwayGroupEntity> zjnRoadwayGroupRepository, ISqlSugarRepository<ZjnWmsRoadwayInboundPlanEntity> zjnRoadwayInboundPlanRepository
            ,ISqlSugarRepository<ZjnRoadwayGroupDetailsEntity> zjnRoadwayGroupDetailsRepository,
        ...
        {
            var output = (await _zjnRoadwayGroupRepository.GetFirstAsync(p => p.Id == id)).Adapt<ZjnRoadwayGroupInfoOutput>();
            
            var zjnRoadwayGroupDetailsList = await _zjnRoadwayGroupDetailsRepository.GetListAsync(w => w.RoadwayCode == output.roadwayNo);
            output.zjnRoadwayGroupDetailsList = zjnRoadwayGroupDetailsList.Adapt<List<ZjnRoadwayGroupDetailsInfoOutput>>();
            return output;
        }
		...
		/// <returns></returns>
        [HttpGet("")]
        public async Task<dynamic> GetList([FromQuery] ZjnRoadwayGroupListQueryInput input)
        {
            var sidx = input.sidx == null ? "F_Id" : input.sidx;
            var data = await _zjnRoadwayGroupRepository.AsSugarClient().Queryable<ZjnRoadwayGroupEntity>()
                ...
                return PageResult<ZjnRoadwayGroupListOutput>.SqlSugarPageResult(data);
        }
        ...
        /// <returns></returns>
        [HttpPost("")]
        public async Task Create([FromBody] ZjnRoadwayGroupCrInput input)
        {
            var userInfo = await _userManager.GetUserInfo();
            var entity = input.Adapt<ZjnRoadwayGroupEntity>();
            ...
            try
            {
                ...
                var zjnRoadwayGroupDetailsEntityList = input.zjnRoadwayGroupDetailsList.Adapt<List<ZjnRoadwayGroupDetailsEntity>>();
                if(zjnRoadwayGroupDetailsEntityList != null)
                {
                    ...
                    {
                        ZjnRoadwayGroupDetailsEntity detailsEntity = zjnRoadwayGroupDetailsEntityList.Where(x => x.EnabledMark == 1).OrderBy(x => x.RoadwayDetailsGrade).ToList()[0];
