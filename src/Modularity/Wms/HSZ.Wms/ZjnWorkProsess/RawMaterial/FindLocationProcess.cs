...
namespace HSZ.Wms.ZjnWorkProsess
{
    ...
    public class FindLocationProcess : IFindLocationProcess, IDynamicApiController, ITransient
    {
        ...
        /// <returns></returns>
        [HttpPost("WmsAllotLocationTask/{trayNo}/{deviceId}")]
        public async Task WmsAllotLocationTask(string trayNo, string deviceId)
        {
            //var task = await _zjnTaskListRepository.GetFirstAsync(w => w.TrayNo == trayNo && w.TaskState == 2);
            //if (task == null)
            //{
            //    throw HSZException.Oh("未找到任务信息，无法分配货位");
            //}
            //var detail = await _zjnTaskListDetailsRepository.GetFirstAsync(w => w.TaskId == task.Id && w.TaskDetailsEnd == deviceId);
            //if (detail == null)
            //{
            //    throw HSZException.Oh("未找到子任务信息，无法分配货位");
            //}
            //await _ZjnTaskService.GetNextTaskDetails(detail.TaskDetailsId);
            //下面为测试货位分配逻辑，没有任务参与
            try
            {
                var tray = await _zjnWmsTrayRepository.GetFirstAsync(w => w.TrayNo == trayNo && w.IsDelete == 0);
                if (tray == null)
                {
                    throw HSZException.Oh("未找到托盘数据，无法分配货位");
                }
                var InDevices = await _zjnWmsEquipmentListRepository.GetListAsync(w => w.EquipmentSerialNumber == deviceId);
                if (InDevices.Count == 0)
                {
                    throw HSZException.Oh("未配置RGV接驳台入库口绑定关系");
                }
                var roadwayGroupNo = await RedisHelper.HGetAsync("roadwayGroupNo", deviceId);
                var aisleCount = await RedisHelper.HGetAsync<int>("roadwayGroupNo", roadwayGroupNo);
                if (string.IsNullOrEmpty(roadwayGroupNo))
                {
                    var group = await _zjnWmsEquipmentListRepository.GetFirstAsync(w => w.EquipmentSerialNumber == InDevices[0].TheBinding);
                    if (group == null)
                    {
                        throw HSZException.Oh("未配置入库口巷道绑定关系");
                    }
                    roadwayGroupNo = group.TheBinding[0].ToString();
                    await RedisHelper.HSetAsync("roadwayGroupNo", deviceId, roadwayGroupNo);
                    aisleCount = await _zjnWmsAisleRepository.CountAsync(c => c.AisleNo.StartsWith(roadwayGroupNo));
                    await RedisHelper.HSetAsync("roadwayGroupNo", roadwayGroupNo, aisleCount);
                }
                var currentAisle = await _zjnWmsRoadwayInboundPlanRepository.GetFirstAsync(w => w.RoadwayGroupNo == roadwayGroupNo);
                if (currentAisle == null)
                {
                    throw HSZException.Oh("未配置巷道入库均衡策略");
                }
                var locationGroup = await _zjnWmsLocationGroupRepository.GetFirstAsync(w => w.GroupName == tray.Type.ToString() && w.EnabledMark == 1);
                ...
                var locations = await _zjnWmsLocationRepository.AsQueryable()
                    .InnerJoin<ZjnWmsAisleEntity>((a, b) => a.AisleNo == b.AisleNo)
                        ...
                        .OrderBy(a => a.Layer)
                        .ToListAsync();
                var locDic = locations.GroupBy(g => g.AisleNo).ToDictionary(k => k.Key, v => v.ToList());//直接找出3个巷道的数据
                if (locDic.Count == 0)
                {
                ...
                else
                {
                    ...
                    int cur = num + 1;
                    while (cur != num && !locDic.ContainsKey(currentAisle.NowroadwayGroup))
                    {
                        if (cur >= aisleCount)
                        {
                            cur = 1;
                ...
                _db.BeginTran();
                ZjnWmsLocationEntity location = locations[0];
                await _zjnWmsLocationAutoService.UpdateLocationStatus(location.LocationNo, LocationStatus.Order);
                if (tray.Type == 2)
                {
                    int index = -1;
                    var locationDic = locations.ToDictionary(k => k.LocationNo, v => v);
                    var locno = string.Empty;
                    for (int i = 0; i < locations.Count; i++)//箔材托盘要求相邻货位空闲才能放入,正常顺序不会连续循环
                    {
                        locno = $"{locations[i].ByWarehouse}-{locations[i].AisleNo}-{locations[i].Row.ToString("d3")}-{(locations[i].Cell + 1).ToString("d3")}-{locations[i].Layer.ToString("d3")}";
                        if (locationDic.ContainsKey(locno))
                        {
                            index = i;
                            break;
                        }
                    ...
                    if (index < 0)
                    {
                        throw HSZException.Oh("未找到箔材托盘货位");
                    }
                    await _zjnWmsLocationAutoService.UpdateLocationStatus(locno, LocationStatus.Order);
                }

                //int code = Convert.ToInt32(currentAisle.NowroadwayGroup.Substring(1));
                //if (code >= aisleCount)
                //{
                ...
                //else
                //{
                //    currentAisle.NowroadwayGroup = currentAisle.NowroadwayGroup[0] + (code + 1).ToString();
                //}
        ...
        public async Task<ZjnWmsTaskDetailsInfoOutput> WmsAllotLocationTask(ZjnWmsTaskDetailsInfoOutput dto)
        {
            try
            {
                ...
                var roadwayGroupNo = await RedisHelper.HGetAsync("roadwayGroupNo", dto.taskDetailsStart);//接驳台位置
                var aisleCount = await RedisHelper.HGetAsync<int>("roadwayGroupNo", roadwayGroupNo);//巷道号
                if (string.IsNullOrEmpty(roadwayGroupNo))
                {
                    var group = await _zjnWmsEquipmentListRepository.GetFirstAsync(w => w.EquipmentSerialNumber == InDevices[0].TheBinding);
                    if (group == null)
                    ...
                    roadwayGroupNo = group.TheBinding[0].ToString();
                    await RedisHelper.HSetAsync("roadwayGroupNo", dto.taskDetailsStart, roadwayGroupNo);
                    aisleCount = await _zjnWmsAisleRepository.CountAsync(c => c.AisleNo.StartsWith(roadwayGroupNo));
                    await RedisHelper.HSetAsync("roadwayGroupNo", roadwayGroupNo, aisleCount);
                }
                var currentAisle = await _zjnWmsRoadwayInboundPlanRepository.GetFirstAsync(w => w.RoadwayGroupNo == roadwayGroupNo);
                ...
                var locations = await _zjnWmsLocationRepository.AsQueryable()
                    .InnerJoin<ZjnWmsAisleEntity>((a, b) => a.AisleNo == b.AisleNo)
                        ...
                        .PartitionBy(a => new { a.Layer }).Take(_locationGenerator.MaxPerLayer)//用PartitionBy必须Take，源码默认了Take(1)
                                                                                               ...
                                                                                               //.OrderByIF(roadwayGroupNo == "B", a => a.Cell, OrderByType.Asc)
                        .OrderBy(a => a.Cell, OrderByType.Desc)
                        .OrderBy(a => a.Priority, OrderByType.Desc)
                        .OrderBy(a => a.Layer)
                        .ToListAsync();
                ...
                if (!string.IsNullOrEmpty(nextEntity.TaskDetailsEnd))//出现了连续获取任务导致重复分配的情况，直接返回
                {
                    return dto;
                }
        ...
        private async Task<(ZjnWmsLocationEntity, string)> AllotLocation(List<ZjnWmsLocationEntity> locations, string goodsCode, int? trayType)
        {
            ...
            string moveLocationNo = string.Empty;
            ...
            foreach (var location in locations)
            {
                ...
                if (trayType == 2)
                {
                    nearLocationNo = $"{location.ByWarehouse}-{location.AisleNo}-{location.Row.ToString("d3")}-{(location.Cell + 1).ToString("d3")}-{location.Layer.ToString("d3")}";
                    if (!locDic.ContainsKey(nearLocationNo))//相邻货位有货找下一个
                    {
                        continue;
                    }
                }
                if (side.LeftSide == 1 && side.RightSide == 1)
                {
                    return (location, nearLocationNo);
                }
                if (side.LeftSide == 1 && location.Row == 2)
                {
                    return (location, nearLocationNo);
                }
                if (side.RightSide == 1 && location.Row == 3)
                {
                    return (location, nearLocationNo);
                }
                    ...
                    if (location.Row == 1)
                    {
                        ...
                        if (trayType == 1)
                        {
                            if (!leftOuterDic.ContainsKey(outerLocationNo))//外面不是空货位
                            {
                                ...
                                {
                                    needMove = true;
                                    moveLocationNo = outerLocationNo;
                        ...
                        {
                            var nearOuterLocationNo = $"{location.ByWarehouse}-{location.AisleNo}-002-{(location.Cell + 1).ToString("d3")}-{location.Layer.ToString("d3")}";
                            if (!leftOuterDic.ContainsKey(outerLocationNo) || !leftOuterDic.ContainsKey(nearOuterLocationNo))//外面不是空货位
                            {
                                if (selectedLocation == null)
                                {
                                    needMove = true;
                ...
                if (side.RightSide > 1)
                {
                    if (location.Row == 4)
                    {
                        ...
                        {
                            var nearOuterLocationNo = $"{location.ByWarehouse}-{location.AisleNo}-003-{(location.Cell + 1).ToString("d3")}-{location.Layer.ToString("d3")}";
                            if (!rightOuterDic.ContainsKey(outerLocationNo) || !rightOuterDic.ContainsKey(nearOuterLocationNo))//外面不是空货位
                            {
                                if (selectedLocation == null)
                                {
                                    needMove = true;
                ...
                if (side.LeftSide > 1)
                {
                    if (location.Row == 2)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                return (location, trayType == 2 ? nearLocationNo : string.Empty);
                            }
                            else
                            {
                                if (selectedLocation == null)
                                {
                                    selectedLocation = location;
                                    selectedNearLocationNo = trayType == 2 ? nearLocationNo : string.Empty;
                                }
                                continue;
                            }
                        }
            ...
            if (needMove)
            {
                ...
                {
                    taskName = "分配货位移库任务",
                    positionCurrent = selectedLocation.LocationNo,
                    positionFrom = moveLocationNo,
                    priority = 3
                };
        ...
        private async Task<ZjnWmsLocationEntity> AllotLocationV2(List<ZjnWmsLocationEntity> locations, string goodsCode)
        {
            ...
            Dictionary<string, string> rightOuterDic = null;
            Dictionary<string, object> locGoodsDic = null;
            Dictionary<string, object> lockDic = null;
            var referInfo = _locationGenerator.ReferInfo;
            string moveLocationNo = string.Empty;
            ZjnWmsLocationEntity selectedLocation = null;
            foreach (var location in locations)
            {
                ...
                if (lockDic == null)
                {
                    lockDic = await _zjnWmsLocationRepository.AsQueryable().Where(w => w.Row == 1 || w.Row == 4)
                        .Where(w => w.LocationStatus == "8").ToDictionaryAsync(k => k.LocationNo, v => v);
                }
                ...
                if (side.LeftSide > 1)
                {
                    if (location.Row == 2)
                    {
                        ...
                        {
                            if (locGoodsDic[location.LocationNo].ToString() == goodsCode
                                && !lockDic.ContainsKey(location.LocationNo))//增加锁定过滤，即里面的货物要出则不用放，否则也是无谓增加移库动作
                            {
                                return location;
                            }
                            else
                ...
                if (side.RightSide > 1)
                {
                    if (location.Row == 3)
                    {
                        ...
                        {
                            if (locGoodsDic[location.LocationNo].ToString() == goodsCode
                                && !lockDic.ContainsKey(location.LocationNo))
                            {
                                return location;
                            }
                            else
