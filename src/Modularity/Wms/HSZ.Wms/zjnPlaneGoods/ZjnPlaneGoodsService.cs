...
namespace HSZ.wms.ZjnPlaneGoods
{
    ...
    /// 平面库物料基础信息服务
    /// </summary>
    [ApiDescriptionSettings(Tag = "wms", Name = "ZjnPlaneGoods", Order = 200)]
    [Route("api/wms/[controller]")]
    public class ZjnPlaneGoodsService : IZjnPlaneGoodsService, IDynamicApiController, ITransient
    {
        ...
        /// <summary>
        /// 初始化一个<see cref="ZjnPlaneGoodsService"/>类型的新实例
        /// </summary>
        public ZjnPlaneGoodsService(ISqlSugarRepository<ZjnPlaneGoodsEntity> zjnPlaneGoodsRepository, ISqlSugarRepository<DictionaryDataEntity> dictionaryDataRepository, ISqlSugarRepository<ZjnWmsOperationLogEntity> zjnPlaneOperationLogRepository,
        ...
        {
            var output = (await _zjnPlaneGoodsRepository.GetFirstAsync(p => p.Id == id)).Adapt<ZjnPlaneGoodsInfoOutput>();
            return output;
        }
		...
		/// <returns></returns>
        [HttpGet("")]
        public async Task<dynamic> GetList([FromQuery] ZjnPlaneGoodsListQueryInput input)
        {
            var sidx = input.sidx == null ? "F_Id" : input.sidx;
            var data = await _zjnPlaneGoodsRepository.AsSugarClient().Queryable<ZjnPlaneGoodsEntity>().Where(x => x.IsDelete == 0)
        ...
        /// <returns></returns>
        [HttpPost("")]
        public async Task Create([FromBody] ZjnPlaneGoodsCrInput input)
        {
            var userInfo = await _userManager.GetUserInfo();
            var entity = input.Adapt<ZjnPlaneGoodsEntity>();
            ...
            var list = _zjnPlaneGoodsRepository.AsQueryable().Where(x => x.GoodsCode == entity.GoodsCode);
            if (list.Count() > 0)
            {
            ...
            var isOk = await _zjnPlaneGoodsRepository.AsInsertable(entity).IgnoreColumns(ignoreNullColumn: true).ExecuteCommandAsync();
		...
		/// <returns></returns>
        [NonAction]
        public async Task<dynamic> GetNoPagingList([FromQuery] ZjnPlaneGoodsListQueryInput input)
        {
            var sidx = input.sidx == null ? "F_Id" : input.sidx;
...
) => new ZjnPlaneGoodsListOutput
{
    ...
    F_ShelfLife=a.ShelfLife
        ...
        public async Task<dynamic> Export([FromQuery] ZjnPlaneGoodsListQueryInput input)
        {
            var userInfo = await _userManager.GetUserInfo();
            var exportData = new List<ZjnPlaneGoodsListOutput>();
            if (input.dataType == 0)
            {
                var data = Clay.Object(await this.GetList(input));
                exportData = data.Solidify<PageResult<ZjnPlaneGoodsListOutput>>().list;
            }
            else
            {
            ...
            ///编号转换名称
            List<ZjnPlaneGoodsListOutput> listPla = new List<ZjnPlaneGoodsListOutput>();
            ...
            ExcelExportHelper<ZjnPlaneGoodsListOutput>.Export(listPla, excelconfig, addPath);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task Update(string id, [FromBody] ZjnPlaneGoodsUpInput input)
        {
            var userInfo = await _userManager.GetUserInfo();
            var entity = input.Adapt<ZjnPlaneGoodsEntity>();
            ///查询原先内容
            var entity1 = await _zjnPlaneGoodsRepository.GetFirstAsync(p => p.Id.Equals(id));
            ...
            try
            {
                ...
                _db.BeginTran();
                //修改数据
                var isOk = await _zjnPlaneGoodsRepository.AsUpdateable(entity).ExecuteCommandAsync();
                //新增日子记录
        ...
        public async Task Delete(string id)
        {
            var userInfo = await _userManager.GetUserInfo();
            ZjnWmsOperationLogEntity operationLogEntity = new ZjnWmsOperationLogEntity();
            var entity = await _zjnPlaneGoodsRepository.GetFirstAsync(p => p.Id.Equals(id));
        ...
        public dynamic TheRawMaterial(string fileName)
        {
            ...
            //插入案例
            var UnitName = _dictionaryDataRepository.AsQueryable().Where(s => s.DictionaryTypeId == "326384591566800133").Select(s => new ZjnPlaneGoodsListOutput { UnitName = s.FullName + ":" + s.EnCode+";" }).ToList();
            var GoodsType = _dictionaryDataRepository.AsQueryable().Where(s => s.DictionaryTypeId == "325449144728552709").Select(s => new ZjnPlaneGoodsListOutput { UnitName = s.FullName + ":" + s.EnCode + ";" }).ToList();
            var GoodsState = _dictionaryDataRepository.AsQueryable().Where(s => s.DictionaryTypeId == "326590282281780485").Select(s => new ZjnPlaneGoodsListOutput { UnitName = s.FullName + ":" + s.EnCode + ";" }).ToList();
            var CheckType = _dictionaryDataRepository.AsQueryable().Where(s => s.DictionaryTypeId == "325448312364729605").Select(s => new ZjnPlaneGoodsListOutput { UnitName = s.FullName + ":" + s.EnCode + ";" }).ToList();
            var dataList = new List<ZjnPlaneGoodsListOutput>() { new ZjnPlaneGoodsListOutput() {
            ...
            var addPath = FileVariable.TemporaryFilePath + excelconfig.FileName;
            ExcelExportHelper<ZjnPlaneGoodsListOutput>.Export(dataList, excelconfig, addPath);
        ...
        public async Task<dynamic> RawMaterialWarehousingData(List<ZjnPlaneGoodsListOutput> list)
        {
            var res = await ImportTheRawMaterialDatas(list);
            var addlist = res.First() as List<ZjnPlaneGoodsListOutput>;
            var errorlist = res.Last() as List<ZjnPlaneGoodsListOutput>;
            return new PlaneGoodsImportResultOutput() { snum = addlist.Count, fnum = errorlist.Count, failResult = errorlist, resultType = errorlist.Count < 1 ? 0 : 1 };
        }
        ...
        private async Task<object[]> ImportTheRawMaterialDatas(List<ZjnPlaneGoodsListOutput> list)
        {
            List<ZjnPlaneGoodsListOutput> PlaneGoodsList = list;
            ...
            var _zjnBillsHistoryRepositoryList = await _zjnPlaneGoodsRepository.GetListAsync();//数据集
            ...
            var PlaneGoods = new List<ZjnPlaneGoodsEntity>();
            var userInfo = await _userManager.GetUserInfo();

            foreach (var item in PlaneGoodsList)
            {
                var uentity = new ZjnPlaneGoodsEntity();
            ...
            if (PlaneGoods.Any())
            {
                try
                {
                    ...
                    //新增记录
                    var newEntity = await _zjnPlaneGoodsRepository.AsInsertable(PlaneGoods).ExecuteReturnEntityAsync();
        ...
        public async Task<dynamic> ExportExceptionData(List<ZjnPlaneGoodsListOutput> list)
        {
            var res = await ImportTheRawMaterialDatas(list);
            var errorlist = res.Last() as List<ZjnPlaneGoodsListOutput>;//错误数据
            ...
            var addPath = FileVariable.TemporaryFilePath + excelconfig.FileName;
            ExcelExportHelper<ZjnPlaneGoodsListOutput>.Export(errorlist, excelconfig, addPath);
