...
using static Zjn.Siem.PackWriteD;
...
namespace Zjn.Siem
{
    public class Device
    {
        ...
        public Device(CpuType cpu, string ip, int port, short rack, short sock, string key="",string device="", int timeout = 2000)
        {
            this.Plc  = new S7.Net.Plc(CpuType.S71500, ip, rack, sock);
        ...
        public void Start()
        {
            try
            {
                ...
                this.isLoop = false;
                //防止已经打开
                if (this.Plc.IsConnected) this.Close();
                //启动
                this.Plc.Open();
                ...
                System.Threading.Tasks.Task.Run(() =>
                {
                    try
                    {
                        while (isLoop)
                        {
                            ...
                            {
                                // 定期ping ip
                                this.mIsConnected = Ping(this.Plc.IP, 10000);
        ...
        /// <returns></returns>
        public object ReadObject(string point)
        {
            try
            {
        ...
        public void WriteObject(string point, object obj)
        {
            try
            {
                if (!this.IsConnected) this.Start();
        ...
        public void WriteDbClass(object obj, int db, int start = 0)
        {
            try
            {
                if (!this.IsConnected) this.Start();
        ...
        /// <param name="start"></param>
        /// <returns></returns>
        public T ReadDbClass<T>(int db, int start = 0)
        {
        ...
        {
            return (T)this.ReadDbClass(classname, db, start, listcount);
        }
        ...
        private void SetPackOfDbInfo(object obj, int db, int start = 0)
        {
            try
            {
                ...
                {
                    case "PackTestList":
                        var obj8 = obj as PackTestList;
                        obj8.List.ToList().ForEach(x => { x.SetCfg(this, db, start + x.PackSize * i); i++; });
                        break;
                    case "PackReadList":
        ...
        /// <returns></returns>
        public byte[] ReadDbBytes(int db, int start, int length)
        {
        ...
        public void WriteDbBytes(int db, int start,  byte[] bytes)
        {
            try
            {
                if (!this.IsConnected) this.Start();

                this.Plc.WriteBytesAsync(DataType.DataBlock, db, start, bytes);
            }
            catch
            {
                this.Close();
        ...
        /// <param name="start"></param>
        /// <returns></returns>
        public string ReadDbS7String(int db, int start)
        {
            try
            {
                if (!this.IsConnected) this.Start();
        ...
        public void WriteDbS7String(int db, int start, string data, int max = 0)
        {
            try
            {
                if (!this.IsConnected) this.Start();
